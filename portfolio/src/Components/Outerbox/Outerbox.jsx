...
const Outerbox = () => {
  ...
  const calculatedsliderData=useMemo(()=>{
    
    
    return sliderData.slice(hovered)
  },[hovered])
  function handlehover(obj) {
    const viewable=6
    if(obj ==='right'){
      sethovered(c=>{
        return Math.min(++c,sliderData.length-viewable)
      })
  ...
  return (
    <div className='container'>
      ...
      {calculatedsliderData.map(c=><div className='t' style={{backgroundColor:c.color}}> 
        {c.name}
